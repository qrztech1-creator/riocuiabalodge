...
export async function GET(request: Request) {
  try {
    ...
    const postsHtml = \`
      <div style="width: 100%; max-width: 1224px; margin: 0 auto; display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 32px; padding: 40px 20px; font-family: 'DM Sans', sans-serif;">
        \${posts
          .map(
            (post) => \`
    ...
    // We find the target block and insert the posts inside it in the raw HTML string
    const targetDiv = '<div id="${targetId}"';
    if (html.includes(targetDiv)) {
       html = html.replace(targetDiv, targetDiv + '>' + postsHtml + '<div style="display:none;"');
    }
